...
const store = {
	...
	state: {
		// Start paused until user clicks
		paused: true,
		// Always enabled
		soundEnabled: true,
		menuOpen: false,
...
const isRunning = (state=store.state) => !state.paused; // Remove && !state.menuOpen
...
const appNodes = {
	...
	autoLaunch: '.auto-fire',
	autoLaunchLabel: '.auto-fire-label',
...
function init() {
	...
	// Show start overlay
	if (appNodes.startOverlay) {
		appNodes.startOverlay.classList.remove('hide');
	}
	...
	// Don't start simulation yet - wait for user click
	// togglePause(false);
	toggleSound(true);
	
	// Function to start audio and fireworks
	...
	const startAudio = () => {
		...
		// Start the fireworks simulation
		togglePause(false);
		
		// Hide start overlay
		if (appNodes.startOverlay) {
			appNodes.startOverlay.classList.add('hide');
		}
		...
		if (appNodes.startOverlay) {
			appNodes.startOverlay.removeEventListener('click', startAudio);
		}
	};
	
	// Add click listener to start overlay and document
	if (appNodes.startOverlay) {
		appNodes.startOverlay.addEventListener('click', startAudio);
	}
...
function handlePointerStart(event) {
	...
	// Remove control button click handling - buttons no longer exist
	// if (event.y < btnSize) {
	// 	if (event.x < btnSize) {
	// 		togglePause();
	// 		return;
